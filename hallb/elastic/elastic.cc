...
void PrintUsage() {
	...
	G4cerr << G4endl;
	G4cerr
			<< "  elastic [-n number of events] [-m batch macro ] [-u UIsession ] [-t nThreads] [-p physList ] [-h | --help] "
...
int main(int argc, char **argv) {
	...
	G4int nThreads = 0;
	G4int nEvents = 0;
	G4String output_file = "elastic.root";
	G4int seed = 0;
	...
	cout << " > elastic executable - using options: " << endl;
	...
	runManager->SetUserInitialization(new elastic::DetectorConstruction());

	auto gphysics = new GPhysics(printAvailablePhysics, physListString);
	runManager->SetUserInitialization(gphysics->getPhysList());

	auto actionInitialization = new elastic::ActionInitialization(output_file);
